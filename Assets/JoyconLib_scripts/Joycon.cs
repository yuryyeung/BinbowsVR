    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                ts_de = report_buf[1];
                DebugPrint($"Dequeue. Queue length: {reports.Count}. Packet ID: {report_buf[0]}. Timestamp: {report_buf[1]}. Lag to dequeue: {System.DateTime.Now.Subtract(rep.GetTime())}. Lag between packets (expect 15ms): {rep.GetTime().Subtract(ts_prev)}", DebugType.THREADING);
                ts_prev = rep.GetTime();
            }
